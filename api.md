...
- Description: Get all active pickup locations for storefront checkout.
- Query Params: None
- URL Params: None
- Required Headers: None
...
- Description: Initiate payment for one of the authenticated customer's pending orders.
- Query Params: None
- URL Params: None
- Required Headers: `Content-Type: application/json`, `Cookie: accessToken=<jwt>; better-auth.session_token=<session-token>`
...
- Query Params: `status` (string), `page` (number), `limit` (number), `sortBy` (string), `sortOrder` (`asc` | `desc`)
- URL Params: None
- Required Headers: `Cookie: accessToken=<jwt>; better-auth.session_token=<session-token>`
