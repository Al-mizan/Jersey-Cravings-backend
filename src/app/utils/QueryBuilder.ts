...
> {
    ...
    filter(): this {
        ...
        Object.keys(this.queryParams as Record<string, unknown>).forEach(
            (key) => {
    ...
    dynamicInclude(
        ...
        const includeParam = (this.queryParams as Record<string, unknown>)
    ...
    async exec(extraArgs?: Partial<PrismaFindsManyArgs>): Promise<T[]> {
        if (extraArgs && Object.keys(extraArgs).length > 0) {
            const { orderBy: extraOrderBy, ...restExtraArgs } = extraArgs;
            const mergedOrderBy = this.mergeOrderBy(
                this.query.orderBy as PrismaFindsManyArgs["orderBy"],
                extraOrderBy as PrismaFindsManyArgs["orderBy"],
            );
            ...
            this.query = this.deepMerge(
                ...
                restExtraArgs as unknown as Record<string, unknown>,
            ) as unknown as PrismaFindsManyArgs;

            if (mergedOrderBy !== undefined) {
                this.query.orderBy = mergedOrderBy;
            }
        }
    ...
    private mergeOrderBy(
        currentOrderBy: PrismaFindsManyArgs["orderBy"],
        extraOrderBy: PrismaFindsManyArgs["orderBy"],
    ): PrismaFindsManyArgs["orderBy"] {
        if (currentOrderBy === undefined) {
            return extraOrderBy;
        }

        if (extraOrderBy === undefined) {
            return currentOrderBy;
        }

        const currentOrderByList = Array.isArray(currentOrderBy)
            ? currentOrderBy
            : [currentOrderBy];
        const extraOrderByList = Array.isArray(extraOrderBy)
            ? extraOrderBy
            : [extraOrderBy];

        return [...currentOrderByList, ...extraOrderByList];
    }
